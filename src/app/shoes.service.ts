...
export class ShoesService {
  ...
  getShoeInfo(prdNo: number) {
    return this.http.get(`https://api.allorigins.win/get?url=http://abcmart.a-rt.com/product/info?prdtNo=${prdNo}`).pipe(
  ...
  getShoesInfo(prdNoList: number[]) {
    ...
    prdNoList.map(prdNo => {
      return requests.push(this.http.get(`https://api.allorigins.win/get?url=http://abcmart.a-rt.com/product/info?prdtNo=${prdNo}`).pipe(
        map(response => this.mapData(response))
      ))
    });
